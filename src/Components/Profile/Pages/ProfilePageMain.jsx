            ...
            <Box sx={{ m: 1 }}>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item size={{ xs: 12, sm: 12, md: 6, lg: 3 }}>
                        <ScoreCard
                            title="Top stars FC"
                            value={<div style={{ display: 'flex' }}><DifficultyBadge difficulty={getRulesetStatistics(activeRuleset)?.scores_set?.highlighted_scores?.['top_stars']?.attr_diff?.star_rating || getRulesetStatistics(activeRuleset)?.scores_set?.highlighted_scores?.['top_stars']?.beatmap?.stars} /></div>}
                            score={getRulesetStatistics(activeRuleset)?.scores_set?.highlighted_scores?.['top_stars']}
                        />
                    </Grid>
                    <Grid item size={{ xs: 12, sm: 12, md: 6, lg: 3 }}>
                        <ScoreCard
